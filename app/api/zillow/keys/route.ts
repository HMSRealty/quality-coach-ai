// CRUD for the per-tenant Zillow / RapidAPI key pool.
// GET    → list keys (no plaintext)
// POST   → add a key (label + key)
// PATCH ?id=... → toggle is_active / reset errors / rename
// DELETE ?id=... → remove
...
// Owners + admins only. Sub-users (callers) cannot touch integrations.
async function assertManager(req: NextRequest) {
  const user = await getCaller(req);
  const sb = admin();
  const { data } = await sb.from("profiles").select("role, parent_user_id").eq("id", user.id).maybeSingle();
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: maxRow } = await sb.from("zillow_api_keys")
    ...
    const { error } = await sb.from("zillow_api_keys").insert({
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    if (!id) return NextResponse.json({ ok: false, error: "id required" }, { status: 400 });
    const sb = admin();
    await sb.from("zillow_api_keys").delete().eq("id", id).eq("user_id", user.id);
    return NextResponse.json({ ok: true });
  } catch (e) {
    return NextResponse.json({ ok: false, error: e instanceof Error ? e.message : "Error" }, { status: 500 });
  }
