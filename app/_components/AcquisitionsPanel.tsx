...
const INP: React.CSSProperties = {
  ...
  color: "#15131D", fontSize: 12.5, outline: "none",
};
...
export function AcquisitionsPanel({
  ...
  const generateOfferPDF = () => {
    ...
    const html = `<!doctype html><html><head><meta charset="utf-8"><title>Cash Offer — ${address || "Property"}</title>
      <style>
        *{font-family:Inter,-apple-system,Segoe UI,sans-serif;color:#15131D;box-sizing:border-box}
        ...
        h1{font-size:26px;margin:24px 0 4px} .muted{color:#6B6880;font-size:13px}
        .card{border:1px solid #22222c;border-radius:14px;padding:20px;margin-top:20px}
        .row{display:flex;justify-content:space-between;padding:9px 0;border-bottom:1px solid #F1F2F8;font-size:14px}
      ...
      ${validComps.length ? `<div class="card">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          <tr style="color:#6B6880;text-align:left"><th style="text-align:left;padding:6px 0">Address</th><th style="text-align:left">Layout</th><th style="text-align:right">Sq. Ft.</th><th style="text-align:left;padding-left:10px">Status</th><th style="text-align:right">Value</th></tr>
          ${validComps.map(c => `<tr style="border-top:1px solid #F1F2F8"><td style="padding:6px 0">${c.address || "—"}</td><td>${c.layout || "—"}</td><td style="text-align:right">${c.sqftN ? c.sqftN.toLocaleString() : "—"}</td><td style="padding-left:10px">${c.status}</td><td style="text-align:right;color:#2563EB;font-weight:700">${money(c.valueN)}</td></tr>`).join("")}
        </table>
      </div>` : ""}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      ...
      <div style={{ background: "#FFFFFF", border: "1px solid var(--border-2)", borderRadius: 18, boxShadow: "var(--shadow-sm)", overflow: "hidden" }}>
        ...
        <div style={{ padding: 16, overflowX: "auto" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 10, flexWrap: "wrap" }}>
            <button onClick={addComp} style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "7px 13px", borderRadius: 9, background: "#F1F2F8", border: "1px solid var(--border-2)", color: SKY_600, fontSize: 12.5, fontWeight: 800, cursor: "pointer" }}>
      ...
      <div style={{ background: "#FFFFFF", border: "1px solid var(--border-2)", borderRadius: 18, boxShadow: "var(--shadow-sm)", overflow: "hidden" }}>
        ...
        <div style={{ padding: 20, display: "grid", gridTemplateColumns: "minmax(0,1.1fr) minmax(0,0.9fr)", gap: 20 }} className="ci-grid">
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            {/* Manual ARV input */}
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "9px 14px", borderRadius: 10, background: "#EFF5FF", border: `1px solid ${SKY_600}55` }}>
              <div>
              ...
              <div style={{ display: "inline-flex", alignItems: "center", gap: 3 }}>
                ...
                <input type="number" min={0} step={1000} value={arv || ""} onChange={e => setArv(Math.max(0, Number(e.target.value) || 0))}
                  ...
                  style={{ width: 110, textAlign: "right", padding: "5px 8px", borderRadius: 8, border: `1px solid ${SKY_600}55`, background: "#FFFFFF", color: "#15131D", fontSize: 15, fontWeight: 800, outline: "none" }} />
              </div>
            </div>
      ...
      {/* ── EXIT STRATEGY ── */}
      {arv > 0 && (
        <div style={{ background: "#FFFFFF", border: "1px solid var(--border-2)", borderRadius: 18, padding: 22, boxShadow: "var(--shadow-sm)" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap", marginBottom: 4 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <p style={{ fontSize: 15, fontWeight: 800, color: "#15131D" }}>Does this deal work? — Flip vs BRRRR vs Hold</p>
            </div>
            {strat.best !== "None" && (
      ...
      {/* ── AI Handoff Dossier ── */}
      {(personality || painPoint || bottomLine) && (
        <div style={{ background: "#FFFFFF", border: "1px solid var(--border-2)", borderTop: `3px solid ${SKY}`, borderRadius: 16, padding: 20, boxShadow: "var(--shadow-sm)" }}>
...
function StrategyCard({ title, verdict, metric, metricLabel, note }: { title: string; verdict: Verdict; metric: string; metricLabel: string; note: string }) {
  ...
  return (
    <div style={{ borderRadius: 14, padding: 16, background: "#FFFFFF", border: `1px solid ${verdict === "Works" ? "color-mix(in srgb, #2563EB 35%, transparent)" : "var(--border-2)"}`, borderTop: `3px solid ${c}` }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
        <p style={{ fontSize: 13.5, fontWeight: 800, color: "#15131D" }}>{title}</p>
...
function NumberRow({ label, value, onChange }: { label: string; value: number; onChange: (n: number) => void }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "9px 14px", borderRadius: 10, background: "#FFFFFF", border: "1px solid var(--border-2)" }}>
      <span style={{ fontSize: 12.5, color: "var(--text-2)", fontWeight: 600 }}>{label}</span>
  ...
  return (
    <div style={{ display: "flex", gap: 11, alignItems: "flex-start" }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <p style={{ fontSize: 13.5, lineHeight: 1.5, fontWeight: isMoney ? 800 : 500, color: isMoney ? MONEY : "#15131D" }}>
