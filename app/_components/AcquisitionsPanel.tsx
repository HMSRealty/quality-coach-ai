...
export function AcquisitionsPanel({
  ...
  const generateOfferPDF = () => {
    ...
    const html = `<!doctype html><html><head><meta charset="utf-8"><title>Cash Offer — ${address || "Property"}</title>
      <style>
        *{font-family:Inter,-apple-system,Segoe UI,sans-serif;color:#15302e;box-sizing:border-box}
        body{max-width:720px;margin:40px auto;padding:0 32px}
        .bar{height:6px;background:linear-gradient(135deg,#0e7c6b,#0a5f52);border-radius:6px}
        ...
        .mao{font-size:30px;font-weight:900;color:#0a5f52}
        .sky{color:#0a5f52;font-weight:800}
      </style></head><body>
      <div class="bar"></div>
      ...
      ${validComps.length ? `<div class="card">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          ...
          ${validComps.map(c => `<tr style="border-top:1px solid #F1F5F9"><td style="padding:6px 0">${c.address || "—"}</td><td>${c.layout || "—"}</td><td style="text-align:right">${c.sqftN ? c.sqftN.toLocaleString() : "—"}</td><td style="padding-left:10px">${c.status}</td><td style="text-align:right;color:#0a5f52;font-weight:700">${money(c.valueN)}</td></tr>`).join("")}
        </table>
      </div>` : ""}
      ...
      <div style={{ background: "#fff", border: "1px solid var(--border-2)", borderRadius: 18, boxShadow: "var(--shadow-sm)", overflow: "hidden" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, padding: "16px 20px", borderBottom: "1px solid var(--border-1)", flexWrap: "wrap" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
            {medianValue > 0 && (
              <span style={{ fontSize: 12, fontWeight: 800, color: MONEY, background: "color-mix(in srgb, #0a5f52 12%, transparent)", padding: "4px 11px", borderRadius: 999 }}>
                Median: {money(medianValue)}
              </span>
            )}
            {address && (
              <button onClick={fetchZillowComps} disabled={fetchingComps} title="Auto-fill comps from Zillow"
                style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "5px 12px", borderRadius: 8, border: "1px solid #0e7c6b33", background: "#F0F9FF", color: SKY_600, fontSize: 11.5, fontWeight: 800, cursor: fetchingComps ? "wait" : "pointer" }}>
      ...
      <div style={{ background: "#fff", border: "1px solid var(--border-2)", borderRadius: 18, boxShadow: "var(--shadow-sm)", overflow: "hidden" }}>
        ...
        <div style={{ padding: 20, display: "grid", gridTemplateColumns: "minmax(0,1.1fr) minmax(0,0.9fr)", gap: 20 }} className="ci-grid">
          ...
          <div style={{
            ...
            background: "var(--money-soft)", border: "1px solid color-mix(in srgb, #0a5f52 30%, transparent)",
          }}>
            ...
            <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={generateOfferPDF}
              style={{
                ...
                boxShadow: "0 8px 20px color-mix(in srgb, #0e7c6b 35%, transparent)",
              }}>
      ...
      {arv > 0 && (
        <div style={{ background: "#fff", border: "1px solid var(--border-2)", borderRadius: 18, padding: 22, boxShadow: "var(--shadow-sm)" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap", marginBottom: 4 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <span style={{ width: 30, height: 30, borderRadius: 8, background: "color-mix(in srgb, #0e7c6b 14%, transparent)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                <Calculator size={15} color={SKY_600} />
              </span>
              <p style={{ fontSize: 15, fontWeight: 800, color: "#000" }}>Does this deal work? — Flip vs BRRRR vs Hold</p>
            </div>
            {strat.best !== "None" && (
              <span style={{ fontSize: 11.5, fontWeight: 800, color: MONEY, background: "color-mix(in srgb, #0a5f52 12%, transparent)", padding: "4px 11px", borderRadius: 999 }}>
                Best fit: {strat.best}
              </span>
            )}
          </div>
...
function StrategyCard({ title, verdict, metric, metricLabel, note }: { title: string; verdict: Verdict; metric: string; metricLabel: string; note: string }) {
  ...
  return (
    <div style={{ borderRadius: 14, padding: 16, background: "#fff", border: `1px solid ${verdict === "Works" ? "color-mix(in srgb, #0a5f52 35%, transparent)" : "var(--border-2)"}`, borderTop: `3px solid ${c}` }}>
