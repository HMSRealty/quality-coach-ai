  ...
  return (
    <div style={{ maxWidth: 1180, margin: "0 auto", display: "flex", flexDirection: "column", gap: 18 }} className="animate-in">
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
        ...
        <button onClick={openNew} style={{ display: "inline-flex", alignItems: "center", gap: 7, padding: "10px 16px", borderRadius: 10, background: "linear-gradient(135deg, #3B82F6, #2563EB)", color: "#fff", border: "none", fontSize: 13, fontWeight: 800, cursor: "pointer", boxShadow: "0 8px 20px rgba(59,130,246,0.35)" }}><Plus size={15} /> Add buyer</button>
      </div>
            ...
            <div key={b.id} style={{ background: "#0A0A0E", border: "1px solid var(--border-2)", borderRadius: 14, padding: 16, boxShadow: "var(--shadow-sm)", display: "flex", flexDirection: "column", gap: 8 }}>
              ...
              {(b.areas?.length > 0 || b.property_types?.length > 0) && (
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                  {(b.areas || []).map(a => <span key={a} style={{ fontSize: 10.5, fontWeight: 700, padding: "2px 8px", borderRadius: 999, background: "color-mix(in srgb, #3B82F6 12%, transparent)", color: SKY_600 }}>{a}</span>)}
        ...
        <div onMouseDown={e => { if (e.target === e.currentTarget) setShowForm(false); }}
          ...
          <div style={{ width: "100%", maxWidth: 560, background: "#0A0A0E", borderRadius: 18, boxShadow: "0 24px 60px rgba(15,23,42,0.30)", overflow: "hidden", animation: "modalIn 200ms cubic-bezier(0.16,1,0.3,1) both" }}>
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, padding: "0 20px 20px" }}>
              ...
              <button onClick={save} disabled={saving || !form.name.trim()} style={{ display: "inline-flex", alignItems: "center", gap: 7, padding: "10px 18px", borderRadius: 10, border: "none", cursor: saving ? "wait" : "pointer", background: form.name.trim() ? "linear-gradient(135deg, #3B82F6, #2563EB)" : "#7DD3FC", color: "#fff", fontSize: 13, fontWeight: 800 }}>
