...
export default function ShiftTargetsPage() {
  ...
  const sel: React.CSSProperties = { padding: "8px 10px", borderRadius: 9, border: "1px solid var(--border-2)", background: "#fff", color: "#000", fontSize: 13, outline: "none" };
  const card: React.CSSProperties = { background: "#fff", border: "1px solid var(--border-2)", borderRadius: 16, padding: 20, boxShadow: "var(--shadow-sm)" };
    ...
    <div style={{ maxWidth: 920, margin: "0 auto", display: "flex", flexDirection: "column", gap: 16 }} className="animate-in">
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <div>
          ...
          <p style={{ fontSize: 13, color: "var(--text-2)" }}>Upload a matrix sheet → map the columns → apply each agent&apos;s shift &amp; daily target.</p>
        </div>
      </div>
      ...
      <div style={card}>
        ...
        <label style={{ border: "2px dashed color-mix(in srgb, #0EA5E9 35%, transparent)", borderRadius: 14, padding: "24px 18px", textAlign: "center", cursor: "pointer", background: "#F8FAFC", display: "block" }}>
      ...
      {grid.length > 0 && (
        <div style={card}>
          <p style={{ fontSize: 14, fontWeight: 800, color: "#000", marginBottom: 12, display: "inline-flex", alignItems: "center", gap: 7 }}><Pencil size={15} color={SKY_600} /> 2 · Map columns</p>
      ...
      {preview.length > 0 && (
        <div style={card}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12, flexWrap: "wrap", gap: 10 }}>
            ...
            <button onClick={apply} disabled={applying} style={{ display: "inline-flex", alignItems: "center", gap: 7, padding: "9px 16px", borderRadius: 10, background: "linear-gradient(135deg, #0EA5E9, #0284C7)", color: "#fff", border: "none", fontSize: 13, fontWeight: 800, cursor: applying ? "wait" : "pointer" }}>
              ...
              <tbody>
                {preview.slice(0, 100).map((r, i) => (
                  <tr key={i} style={{ borderTop: "1px solid var(--border-1)" }}>
                    <td style={{ padding: "8px 12px", fontWeight: 700, color: "#000" }}>{r.name}</td>
