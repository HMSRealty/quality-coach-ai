...
function OutboundWebhooks() {
  ...
  const card: React.CSSProperties = { background: "#FFFFFF", border: "1px solid var(--border-2)", borderRadius: 16, padding: 22, boxShadow: "var(--shadow-sm)" };
  const inp: React.CSSProperties = { width: "100%", padding: "9px 11px", borderRadius: 9, border: "1px solid var(--border-2)", background: "#FFFFFF", color: "#15131D", fontFamily: "var(--font-mono)", fontSize: 12.5, outline: "none" };

  return (
    <div style={card}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <p style={{ fontSize: 15, fontWeight: 800, color: "#15131D", display: "inline-flex", alignItems: "center", gap: 8 }}><Webhook size={16} color={SKY_600} /> Outbound Webhooks</p>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 8 }}>
          {campaigns.map(c => (
            <div key={c.id} style={{ display: "grid", gridTemplateColumns: "minmax(120px,180px) 1fr", gap: 10, alignItems: "center" }}>
              <span style={{ fontSize: 13, fontWeight: 700, color: "#15131D", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{c.name}</span>
...
function CopyBtn({ text, label }: { text: string; label?: string }) {
  ...
  return (
    <button onClick={() => { navigator.clipboard.writeText(text); setDone(true); setTimeout(() => setDone(false), 1500); }}
      style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "7px 12px", borderRadius: 9, background: "#FFFFFF", border: "1px solid var(--border-2)", color: done ? MONEY : "#000", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>
...
export default function ApiIntegrationsPage() {
  ...
  const card: React.CSSProperties = { background: "#FFFFFF", border: "1px solid var(--border-2)", borderRadius: 16, padding: 22, boxShadow: "var(--shadow-sm)" };
  const codeBox: React.CSSProperties = { margin: 0, padding: 16, borderRadius: 12, background: "#15131D", color: "#22222c", fontFamily: "var(--font-mono)", fontSize: 12.5, lineHeight: 1.6, overflowX: "auto" };
  ...
  return (
    <div style={{ maxWidth: 880, margin: "0 auto", display: "flex", flexDirection: "column", gap: 18 }} className="animate-in">
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <div>
          <h1 style={{ fontSize: 24, fontWeight: 900, color: "#15131D", letterSpacing: "-0.02em" }}>API &amp; Integrations</h1>
        ...
        {freshKey && (
          <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={SPRING}
            ...
            <div style={{ display: "flex", gap: 10, alignItems: "center", marginTop: 10 }}>
              <code style={{ flex: 1, padding: "11px 13px", borderRadius: 10, background: "#F1F2F8", border: "1px solid var(--border-2)", fontFamily: "var(--font-mono)", fontSize: 13, color: "#15131D", overflowX: "auto", whiteSpace: "nowrap" }}>{freshKey}</code>
      ...
      <div style={card}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, marginBottom: 16, flexWrap: "wrap" }}>
          <p style={{ fontSize: 15, fontWeight: 800, color: "#15131D", display: "inline-flex", alignItems: "center", gap: 8 }}><KeyRound size={16} color={SKY_600} /> API Keys</p>
          <div style={{ display: "flex", gap: 8 }}>
            <input value={label} onChange={(e) => setLabel(e.target.value)} placeholder="Key label (e.g. Readymode)"
              style={{ padding: "9px 12px", borderRadius: 10, border: "1px solid var(--border-2)", background: "#FFFFFF", color: "#15131D", fontSize: 13, outline: "none", minWidth: 180 }} />
        ...
        {loading ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {keys.map((k) => (
              <div key={k.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "11px 14px", borderRadius: 11, background: "#F1F2F8", border: "1px solid var(--border-1)", opacity: k.revoked ? 0.55 : 1 }}>
                <KeyRound size={15} color={k.revoked ? "var(--text-3)" : SKY_600} style={{ flexShrink: 0 }} />
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{ fontSize: 13, fontWeight: 700, color: "#15131D" }}>{k.label || "Key"} {k.revoked && <span style={{ fontSize: 10, color: "#DC2626", fontWeight: 800 }}>· REVOKED</span>}</p>
      ...
      <div style={card}>
        <p style={{ fontSize: 15, fontWeight: 800, color: "#15131D", display: "inline-flex", alignItems: "center", gap: 8, marginBottom: 4 }}><Webhook size={16} color={SKY_600} /> Inbound Webhook URL</p>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          <code style={{ flex: 1, padding: "12px 14px", borderRadius: 10, background: "#F1F2F8", border: `1px solid ${SKY}`, fontFamily: "var(--font-mono)", fontSize: 13, color: SKY_600, fontWeight: 700, overflowX: "auto", whiteSpace: "nowrap" }}>{webhookUrl}</code>
        ...
        <div style={{ marginTop: 14, display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 10 }}>
          ...
          <div style={{ padding: "12px 14px", borderRadius: 10, background: "rgba(245,158,11,0.12)", border: "1px solid #FDE68A" }}>
            ...
            <p style={{ fontSize: 11.5, color: "#F59E0B", lineHeight: 1.55 }}>
              When the caller dispositions the call in the dialer, post again to the <strong>same URL</strong> with the same address plus the recording: <code style={{ background: "#FFFFFF", padding: "1px 5px", borderRadius: 4 }}>recording_url</code> (or <code style={{ background: "#FFFFFF", padding: "1px 5px", borderRadius: 4 }}>call_recording_url</code>). We&apos;ll match it to the existing lead, attach the audio, and re-analyze automatically.
            </p>
          </div>
        </div>
      </div>
      ...
      {/* Test webhook */}
      <div style={card}>
        <p style={{ fontSize: 15, fontWeight: 800, color: "#15131D", display: "inline-flex", alignItems: "center", gap: 8, marginBottom: 4 }}><Send size={16} color={SKY_600} /> Test Webhook</p>
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          <input value={testKey} onChange={(e) => setTestKey(e.target.value)} placeholder="Paste an API key (rt_live_…)"
            style={{ flex: 1, minWidth: 220, padding: "11px 13px", borderRadius: 10, border: "1px solid var(--border-2)", background: "#FFFFFF", color: "#15131D", fontFamily: "var(--font-mono)", fontSize: 13, outline: "none" }} />
      ...
      <div style={card}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
          <p style={{ fontSize: 15, fontWeight: 800, color: "#15131D", display: "inline-flex", alignItems: "center", gap: 8 }}><Terminal size={16} color={SKY_600} /> JSON Payload</p>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", margin: "16px 0 12px" }}>
          <p style={{ fontSize: 13, fontWeight: 800, color: "#15131D" }}>cURL example</p>
