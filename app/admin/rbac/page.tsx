  ...
  return (
    <div style={{ maxWidth: 1100, margin: "0 auto", display: "flex", flexDirection: "column", gap: 24 }} className="animate-in">
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: 14 }}>
        <div>
          <h1 style={{ fontSize: 26, fontWeight: 900, color: "#15131D", letterSpacing: "-0.02em", display: "flex", alignItems: "center", gap: 10 }}>
      ...
      <div style={{ background: "#FFFFFF", border: "1px solid #22222c", borderRadius: 16, overflow: "hidden", boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 700 }}>
            <thead>
              <tr style={{ background: "#F1F2F8" }}>
                <th style={{ padding: "12px 16px", textAlign: "left", fontSize: 10, fontWeight: 800, letterSpacing: "0.06em", textTransform: "uppercase", color: "#6B6880", borderBottom: "2px solid #22222c", minWidth: 160 }}>
                  Permission
                </th>
                {ROLES.map(r => (
                  <th key={r.key} style={{ padding: "12px 16px", textAlign: "center", fontSize: 11, fontWeight: 800, color: "#15131D", borderBottom: "2px solid #22222c", whiteSpace: "nowrap", minWidth: 100 }}>
                    {r.label}
            ...
            <tbody>
              {groups.map(group => (
                <>
                  <tr key={`g-${group}`}>
                    <td colSpan={ROLES.length + 1} style={{ padding: "10px 16px 4px", fontSize: 10, fontWeight: 800, letterSpacing: "0.08em", textTransform: "uppercase", color: SKY_600, background: "#EFF5FF", borderTop: "1px solid #22222c", borderBottom: "1px solid #22222c" }}>
                      {group}
                    </td>
                  </tr>
                  {PERMISSIONS.filter(p => p.group === group).map((perm, pi) => (
                    <tr key={perm.key} style={{ borderBottom: "1px solid #F1F2F8", background: pi % 2 === 0 ? "#fff" : "#FFFFFF" }}>
                      <td style={{ padding: "11px 16px", fontSize: 13, color: "#15131D", fontWeight: 600 }}>
                        {perm.label}
                          ...
                          <td key={role.key} style={{ padding: "11px 16px", textAlign: "center" }}>
                            <button
                              ...
                              style={{
                                ...
                                background: hasIt ? (isLocked ? "#D1FAE5" : "color-mix(in srgb, #2563EB 15%, transparent)") : "#F1F2F8",
      ...
      <p style={{ fontSize: 12, color: "#94A3B8", lineHeight: 1.6 }}>
        <strong style={{ color: "#6B6880" }}>Note:</strong> This matrix controls the UI experience. The authoritative security boundary is enforced by Postgres Row-Level Security policies. Update your <code style={{ background: "#F1F2F8", padding: "1px 5px", borderRadius: 4, fontSize: 11 }}>0002_rls.sql</code> migration to change RLS rules for real enforcement.
      </p>
    </div>
  );
}
